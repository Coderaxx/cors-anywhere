...
function rewriteSetCookieHeader(setCookieHeaders, req) {
  return setCookieHeaders.map(cookie => {
    // Legg til din logikk her for å omskrive cookie
    // For eksempel, legge til en sti og sette HttpOnly flagget
    return `${cookie}; Domain=ocelektro.onecontrol.no; Path=/; HttpOnly`;
  });
}
      ...
      pipe: function (proxyReq) {
        ...
        proxyReq.on = function (eventName, listener) {
          ...
          return proxyReqOn.call(this, 'response', function (proxyRes) {
            // Her kan du sjekke og eventuelt omskrive 'Set-Cookie' headeren før den sendes til klienten
            if (proxyRes.headers['set-cookie']) {
              // Omskriv 'Set-Cookie' headeren her
              proxyRes.headers['set-cookie'] = rewriteSetCookieHeader(proxyRes.headers['set-cookie'], req);
            }

            // Resten av koden som sjekker om onProxyResponse returnerer true og håndterer lytteren
            if (onProxyResponse(proxy, proxyReq, proxyRes, req, res)) {
              try {
                listener(proxyRes);
              } catch (err) {
                // Håndter eventuelle feil som før
